...
def function5():
    ...
    z = np.zeros_like(np.arange(4*5).reshape(4,5)) #wrtie your code here
...
def function7():
    ...
    return np.full_like(x, dtype=x.dtype, fill_value=0) #write your code here
...
def function8():
    ...
    x = np.full((2,5), dtype=np.uint32, fill_value=6) #write your code here
...
def function18():
    ...
    x = np.count_nonzero(np.intersect1d(students, piaic) >= 1) # Write you code Here
    return x
...
def function19():
    ...
    X = np.arange(1,5*5+1).reshape((5,5))  # Write your code here
